...
export default (outRootDir, encoding = 'utf-8', userSourcemapOptions) => (input) => {
  return function write(log) {
    ...
    return Promise.all(
      input.map((file) => {
        ...
        // /beep/boop/src/beep/index.js -> /beep/boop/src/beep
        const inDir = path.dirname(file.path);
        // /beep/boop/src/beep -> ../build/beep
        const relativePath = path.relative(inDir, outRootDir);
        // ../build/beep -> /beep/boop/build/beep
        const outDir = path.resolve(inDir, relativePath);
